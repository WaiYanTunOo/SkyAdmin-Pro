...
**Current version:** `0.3.1` · **Primary platform:** Windows desktop
...
| Area | Status |
|------|--------|
| Windows portable exe + installer | ✅ `build.ps1`, `build-installer.ps1`, `dist\SkyAdminPro-Setup-0.3.1.exe` |
...
| ~690 | `skyadmin-worker/src/routes/admin.ts` | HTML/JS embedded in TS string |
...
services/license/           verify.py, sync.py, machine.py  (thin facade in license.py)
```
...
| 11.2 | **Windows code signing** | signtool in `build-installer.ps1` |
...
| 11.5 | **Changelog** | `CHANGELOG.md` from release tags |
| 11.6 | **Publish pipeline** | build → release_check → upload → `publish_update.py` |
...
- [x] Signing guide (`packaging/SIGNING.md`)
...
- [x] Database Tasks lazy tabs — Courier, Monthly Tax, Renewals, Pipeline (Phase 9.1)
