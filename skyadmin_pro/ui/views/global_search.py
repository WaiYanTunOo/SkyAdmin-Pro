...
class GlobalSearchDialog(ctk.CTkToplevel):
    ...
    def __init__(self, app: object) -> None:
        ...
        ctk.CTkLabel(input_frame, text="Search:", font=ctk.CTkFont(size=13)).grid(
        ...
        search_entry = themed_entry(
            input_frame, textvariable=self.search_var, placeholder_text="Type to search clients, tasks, documents, contacts…"
        )
        ...
        for label, value in [("All", "all"), ("Clients", "clients"), ("Tasks", "tasks"), ("Docs", "docs"), ("Contacts", "contacts")]:
            ctk.CTkRadioButton(
                filter_frame,
                text=label,
    ...
    def _run_search(self) -> None:
        ...
        if not query:
            self.feedback.set("Type at least 2 characters to search.", "info")
            return

        if len(query) < 2:
            self.feedback.set("Type at least 2 characters.", "info")
            return

        filter_type = self._filter.get()
        results = self._search_all(query, filter_type)
    ...
    def _render_results(self, results: list[dict], query: str) -> None:
        if not results:
            self.feedback.set("No results found.", "info")
            return

        self.feedback.set(f"{len(results)} result(s) found.", "success")
        ...
        for i, item in enumerate(results):
            ...
            if item["subtitle"]:
                ctk.CTkLabel(
                    row, text=item["subtitle"], anchor="w",
